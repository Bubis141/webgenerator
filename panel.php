<?php
    ...
    if(isset($_SESSION['user']) && !empty($_SESSION['user'])){
        ...
        if(isset($_POST['btn'])){
            ...
            $web = $_POST['web'];
            if($web == "" ){
                $msg = "No se ingreso a una web";
            }else{
                $web = $user.$web;
                $sql = "SELECT dominio FROM `webs` WHERE `dominio` = '$web' ";
                $response =  mysqli_query($connection,$sql);
                if( mysqli_num_rows($response) >= 1){
                    $msg = "El Dominio se a registrado anteriormente";
                }else{
                    $sql = "INSERT INTO `webs` (`idUsuario`,`dominio`) VALUES('$user','$web')";
                    $response =  mysqli_query($connection,$sql);
                    if ($response) {
                        $msg = "El registro de la web a sido exitoso";
                        echo shell_exec("./wix.sh '$web' '$web' ");
                        $domainList =  getDomainList($connection);
                    }else {
                        echo "Error en registrarse en la web" . mysqli_error($connection);
                    }
                }
        ...
        if(isset($_POST['delete'])){
            ...
            $response =  mysqli_query($connection,$sql);
            if ($response) {
                $msg = "Se a eliminodo la web correctamente";
                $domainList =  getDomainList($connection);
            }else{
                $msg = "A surgido un error al eliminarse la web";
            }
        }
