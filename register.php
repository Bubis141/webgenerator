<?php
    ...
    if(isset($_POST['btn'])){
        ...
        if( !empty($response) AND  mysqli_num_rows($response) >= 1 ){
            $msg .= "El email que quiere ingresar ya esta registrado, ingrese uno nuevo ";
            $validation = false;
        }

        if($password != $password1 ){
            $msg .= "Sus contraseñas no son indenticas ";  
            $validation = false;
        }

        if($password == "" OR $email == "" ){
            $msg .= "Los campos de testo no pueden estar vacios ";  
            $validation = false;
        }
